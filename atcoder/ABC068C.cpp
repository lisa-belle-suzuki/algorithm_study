...
int main(){
    int N,M,a,b;
    cin >> N >> M;
    bool ship_from_one_to[N+2];
    bool ship_to_N_from[N+2];
    rep(i,N+2){
        ship_from_one_to[i] = false;
        ship_to_N_from[i] = false;
    }
    rep(i,M){
        cin >> a >> b;
        if (a==1) ship_from_one_to[b]=true;
        if (b==N) ship_to_N_from[a]=true;
    }
    for(int i=2;i<N;i++){
        if (ship_from_one_to[i] && ship_to_N_from[i]){
            cout << "POSSIBLE" << endl;
            return 0;
        }
    }
