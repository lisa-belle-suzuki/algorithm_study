...
int main(){
    ...
    cin >> N >> K;
    if (K<=N){
        double p = (double)(N-K+1)/(double)N;
        llRep(i,K-1){
            ll min_m = minimum_m(K,i);
            p += (double)pow(0.5,(double)min_m)/(double)N;
        }
        cout << std::fixed << std::setprecision(12) << p << endl;
        return 0;
    }else{
        double p = 0;
        llRep(i,N){
            ll min_m = minimum_m(K,i);
            p += ( (double)1 / pow(2,(double)min_m) ) / (double)N;
        }
        cout << std::fixed << std::setprecision(12) << p << endl;
        return 0;
    }
