...
int win(ll a, ll b, ll a_strongest, ll b_strongest){
    while(a>0 && a_strongest>0){
        a -= b_strongest;
        a_strongest -= b;
    }
    if (a<=0 && a_strongest<=0) return 0;
...
int main(){
    ...
    llrep(i,N){
        cin >> a >> b;
        ll win_lose = win(a, b ,a_strongest, b_strongest);
        if (win_lose==1){//new one wins
            a_strongest = a;
            b_strongest = b;
            strongest_idx = i+1;
            if(draw==true) draw=false;
        }else if (win_lose==0){//draw
            draw=true;
        }
        // original one wins: do nothing
    }

    if (draw==true){
        cout << -1 << endl;
    }else{
        cout << strongest_idx << endl;
    }
