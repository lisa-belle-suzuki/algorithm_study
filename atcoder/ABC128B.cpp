...
bool pairCompare(pair<int, pair<string, int> > a, pair<int, pair<string, int> > b) {
...
vector< pair<int, pair<string, int> > > restaurant;
...
int main(){
    ...
    rep(i,N){
        cin >> S >> p;
        restaurant.push_back( make_pair(i, make_pair(S,p) ) );
    }
    sort(restaurant.begin(), restaurant.end(), pairCompare);

    rep(i,N){
        cout << restaurant[i].first + 1 << endl;
    }
